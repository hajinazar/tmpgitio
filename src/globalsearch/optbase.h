...
public:
  ...
  /**
   * Types of the features in a multi-objective run
   */
  enum FeatureType
  {
  ...
  */
   ...
   *                       in optimization, 1 means only
   *                       hardness consideration; and -1.0 is the case that no hardness
   *                       calculation is performed.
   * @param features_num (Optional) The number of features introduced by
  ...
  /// Should a redo for structure dismissed by filtration feature be considered?
  bool m_featuresReDo;

  /// Perform feature calculations in a multi-objective search
  bool m_calculateFeatures;

  /// Multi-objective functions for the run
  void setFeaturesNum(int i)         {m_features_num = i;};
  void setFeaturesOpt(FeatureType f) {m_features_opt.push_back(f);};
  void setFeaturesExe(QString f)     {m_features_exe.push_back(f);};
  void setFeaturesOut(QString f)     {m_features_out.push_back(f);};
  void setFeaturesWgt(double f)      {m_features_wgt.push_back(f);};
  int         getFeaturesNum()       const {return m_features_num;};
  FeatureType getFeaturesOpt(int i)  const {return m_features_opt.at(i);};
  QString     getFeaturesExe(int i)  const {return m_features_exe.at(i);};
  QString     getFeaturesOut(int i)  const {return m_features_out.at(i);};
  double      getFeaturesWgt(int i)  const {return m_features_wgt.at(i);};
  //
  void resetFeatures() { m_features_num = 0; m_features_opt.clear(); m_features_exe.clear();
    m_features_out.clear(); m_features_wgt.clear();};
  /// Feature list-related: this holds a list of all feature-related entries
  ///      with the cli input format, i.e., "opt_type script_name filename weight"
  void    featureListAdd(QString f) {m_features_lst.push_back(f);};
  QString featureListGet(int i)     {return m_features_lst.at(i);};
  void    featureListClear()        {m_features_lst.clear();};
  void    featureListRemove(int i)  {m_features_lst.removeAt(i);};
...
signals:
   ...
   * @param s Structure
   */
  void doneWithFeatures(Structure* s);
...
protected:
  ...
  int                m_features_num; // total number of features (internal variable), default = 0
  QStringList        m_features_exe; // location of external scripts for each feature
  QList<FeatureType> m_features_opt; // type of optimization: minimization/maximization/filtration/hardness
  QList<double>      m_features_wgt; // weight for each feature
  QStringList        m_features_out; // output file name produced by the external script for each feature
  QStringList        m_features_lst; // list of feature inputs in xtalopt.in file (internal variable)
