...
void QueueManager::updateStructureFeatureStatus(Structure* s)
{
  ...
  if (s->getStrucFeatStatus() == Structure::FS_Retain)
    ...
    s->setStatus(Structure::FeatureFail);
...
void QueueManager::handleDismissFeature_(Structure* s)
{
  ...
  if (m_opt->m_featuresReDo && s->getStrucFeatFailCt() == 0)
  {
    ...
    if (OptBase::FailActions(m_opt->failAction) == OptBase::FA_Randomize) {
      ...
      locker.unlock();
      m_opt->replaceWithRandom(s, tr("failed feature calculation"));
      ...
      s->setStatus(Structure::Empty);
      locker.unlock();
      m_opt->replaceWithOffspring(s, tr("failed feature calculation"));
      s->setStatus(Structure::Restart);
      emit structureUpdated(s);
      return;
    }
