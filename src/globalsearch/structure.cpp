...
Structure& Structure::operator=(const Structure& other)
{
  if (this != &other) {
    ...
    m_strucFeatValues = other.m_strucFeatValues;
    m_strucFeatStatus = FeatStatus(other.m_strucFeatStatus);
    m_strucFeatFailCt = other.m_strucFeatFailCt;
  }
...
Structure& Structure::operator=(Structure&& other) noexcept
{
  if (this != &other) {
    ...
    m_strucFeatValues = std::move(other.m_strucFeatValues);
    m_strucFeatStatus = std::move(FeatStatus(other.m_strucFeatStatus));
    m_strucFeatFailCt = std::move(other.m_strucFeatFailCt);
  }
...
void Structure::writeStructureSettings(const QString& filename)
{
  ...
  // Features (multi-objective run): write current info to structure.state file
  settings->beginWriteArray("features");
  for (size_t i = 0; i < getStrucFeatNumber(); i++) {
    settings->setArrayIndex(i);
    settings->setValue("value", QString::number(getStrucFeatValues(i)));
  }
  settings->endArray();
  settings->setValue("features_state", getStrucFeatStatus());
  settings->setValue("features_failcount", getStrucFeatFailCt());
  ...
  // Features (multi-objective run): write history to structure.state file
  settings->beginWriteArray("features");
  for (int i = 0; i < getStrucHistFeatNumber(); i++) {
    settings->setArrayIndex(i);
    settings->beginWriteArray("value");
    for (int j = 0; j < getStrucHistFeatValues(i).size(); j++) {
      settings->setArrayIndex(j);
      settings->setValue("value", getStrucHistFeatValues(i).at(j));
    }
    settings->endArray();
    settings->setValue("state", getStrucHistFeatStatus(i));
    settings->setValue("failcount", getStrucHistFeatFailCt(i));
  }
  settings->endArray();
                                      ...
                                      const bool readCurrentInfo)
{
  ...
  if (loadedVersion >= 1) { // Version 0 uses save(QTextStream)
    ...
    // Features (multi-objective run): read current info from structure.state file
    resetStrucFeat();
    size = settings->beginReadArray("features");
    for (int i = 0; i < size; i++) {
      settings->setArrayIndex(i);
      setStrucFeatValues(settings->value("value").toDouble());
    }
    settings->endArray();
    setStrucFeatStatus(FeatStatus(settings->value("features_state", 0).toInt()));
    setStrucFeatFailCt(settings->value("features_failcount", 0).toInt());
    ...
    // Features (multi-objective run): read history from structure.state file
    size = settings->beginReadArray("features");
    resetStrucHistFeat();
    for (int i = 0; i < size; i++) {
      settings->setArrayIndex(i);
...
void Structure::updateAndAddFeaturesToHistory(Structure* s)
{
  s->setStrucHistFeatValues(s->getStrucFeatValuesVec());
  s->setStrucHistFeatStatus(s->getStrucFeatStatus());
  s->setStrucHistFeatFailCt(s->getStrucFeatFailCt());
}
