...
public:
  ...
  /**
   * Enum possible feature calculation outcomes for a structure
   */
  enum FeatStatus
  {
    /** Features are not calculated */
    FS_NotCalculated = 0,
    /** Features are calculated, structure should be kept */
    FS_Retain,
    /** Features calculated, structure dismissed by a filtration feature */
    FS_Dismiss,
    /** Feature calculations failed (bad output file, ...) */
    FS_Fail
  };
  ...
  // we don't reset feature fail count; it might be a redone structure which shouldn't be repeated!
  void      resetStrucFeat() {m_strucFeatValues.clear(); m_strucFeatStatus = Structure::FS_NotCalculated;};
  //
  ...
  QList<double> getStrucHistFeatValues(int i) {return m_hist_strucFeatValues.at(i);};
  FeatStatus    getStrucHistFeatStatus(int i) {return m_hist_strucFeatStatus.at(i);};
  int           getStrucHistFeatFailCt(int i) {return m_hist_strucFeatFailCt.at(i);};
  void          setStrucHistFeatValues(QList<double> v) {m_hist_strucFeatValues.push_back(v);};
  void          setStrucHistFeatStatus(FeatStatus v)    {m_hist_strucFeatStatus.push_back(v);};
  void          setStrucHistFeatFailCt(int v)           {m_hist_strucFeatFailCt.push_back(v);};
  ...
  */
  ...
  /** Add features info to history of the structure
   * @param s Structure whose feature-related info is added to history
   */
...
signals:
  ...
  /**
   * Emitted when a feature's output file is found for a structure
   *
   */
  void featureOutputExists();
...
protected:
  ...
  std::atomic<FeatStatus> m_strucFeatStatus;
  QList<double>           m_strucFeatValues;
  int                     m_strucFeatFailCt;
  // Feature history parameters
  QList<FeatStatus>    m_hist_strucFeatStatus;
  QList<QList<double>> m_hist_strucFeatValues;
  QList<int>           m_hist_strucFeatFailCt;
